...
U-Net \cite{Ronneberger2015} & 0.89 & 0.87 & 0.91 \\
Cellpose \cite{Stringer2021} & 0.91 & 0.90 & 0.92 \\
StarDist \cite{Schmidt2018} & 0.90 & 0.89 & 0.91 \\
\midrule
Hand-crafted morphism chain \cite{partition2024} & 0.93 & 0.92 & 0.94 \\
\textbf{Neural-compiled chain (ours)} & \textbf{0.91} & \textbf{0.90} & \textbf{0.93} \\
\bottomrule
\end{tabular}
\end{table}

The neurally-compiled chain achieves Dice 0.91, within 2\% of the hand-crafted chain (0.93) and matching or exceeding all conventional deep learning methods. The gap reflects imperfect catalyst ordering and suboptimal depth selection ($n = 95$ vs. optimal $n = 100$).
...
\midrule
\textbf{Measured (neural selection)} & \textbf{14.1 nm} & \textbf{14.2$\times$} \\
\textbf{Measured (expert selection)} & \textbf{13.4 nm} & \textbf{14.87$\times$} \\
\bottomrule
\end{tabular}
\end{table}

The neural catalyst selector achieves 14.2$\times$ resolution enhancement compared to 14.87$\times$ for expert selection. The difference arises from slightly suboptimal exclusion factor estimation ($\hat{\epsilon}_{\text{mass}} = 0.27$ vs. true $\epsilon_{\text{mass}} = 0.25$).
...
Standard deviation & $3.2 \times 10^{-4}$ \\
Maximum deviation & $8.7 \times 10^{-4}$ \\
Conservation violation rate & 0\% \\
\bottomrule
\end{tabular}
\end{table}

S-entropy conservation holds to $< 10^{-3}$ for all neurally-compiled chains, validated by the conservation loss term $\loss_{\text{cons}}$ (Eq.~\ref{eq:cons_loss}) and the constrained decoding mechanism (Theorem~\ref{thm:constrained_decoding}).
...
\textbf{Neural compiler (ours)} & \textbf{0.3 s} & \textbf{None} \\
\bottomrule
\end{tabular}
\end{table}

The neural compiler reduces compilation time by three orders of magnitude while eliminating the requirement for domain expertise.
...
\midrule
Nuclei & In-distribution & 0.91 & 0.93 \\
Cell membrane & In-distribution & 0.88 & 0.91 \\
Mitochondria & Out-of-distribution & 0.85 & 0.89 \\
Endoplasmic reticulum & Out-of-distribution & 0.82 & 0.87 \\
\bottomrule
\end{tabular}
\end{table}

Out-of-distribution performance degrades by 3--5\% compared to in-distribution targets, but remains competitive with conventional deep learning methods. This generalization reflects the compositional structure learned during curriculum training (Theorem~\ref{thm:curriculum}): the model applies known catalyst types to novel targets by reasoning about shared biological properties.
...
\textbf{Computational cost}: While compilation is fast (0.3 s), executing the compiled morphism chain---traversing categorical space---requires computation proportional to the categorical distance between observed and target signatures. For deep access ($n_{\text{eff}} > 1000$), execution time may dominate.
...
\textbf{Empirical}: Neurally-compiled morphism chains achieve Dice 0.91 on BBBC039 nuclear segmentation (within 2\% of expert-crafted chains), 14.2$\times$ resolution enhancement (within 5\% of expert-optimized configurations), and S-entropy conservation to $< 10^{-3}$---while reducing compilation time from expert-hours to 0.3 seconds.
