...
\title{Image Contact Maps via S-Entropy Bidirectional Dijkstra:\\
A Theory of Individuation by Negation in Bounded Resolvable Spaces}

\author{Kundai Farai Sachikonye\\
\textit{Technical University of Munich}\\
\textit{AIMe Registry for Artificial Intelligence}\\
\texttt{k.sachikonye@wzw.tum.de}}
...
the \emph{S-entropy state space} $\ScS = [0,1]^3$, whose three axes are
...
$[0,+\infty]$, but we cap it at $1$ by convention: $\Se(A) = \min(1, {S_e}_{\text{raw}}(A))$
where ${S_e}_{\text{raw}}$ is the uncapped ratio.
\end{definition}
...
The \emph{S-entropy state space} is $\ScS = [0,1]^3$ equipped with the Euclidean
metric $\dS(\bS_1, \bS_2) = \norm{\bS_1 - \bS_2}_2$.
\end{definition}
...
we need to handle the fact that intermediate points on a path in $\ScS = [0,1]^3$
...
mathematical intermediary in the decomposition of a path in $\ScS$. A virtual
...
\label{def:sebdgraph}
The \emph{SEBD graph} $G_{\ScS} = (V_\ScS, E_\ScS)$ is the graph on the set of
all S-entropy states of distinguishable regions:
\begin{itemize}
\item $V_\ScS = \{\bS(A) : A \in \PP\}$ (one node per region);
\item $E_\ScS = \{(\bS(A), \bS(B)) : (A,B) \in E_{G_C}\}$ (edges correspond to contacts);
...
\While{$\text{PQ}_F \neq \emptyset$ \textbf{and} $\text{PQ}_B \neq \emptyset$}{
  ...
  \ForEach{neighbor $u$ of $v$ in $G_\ScS$}{
  ...
  \ForEach{neighbor $u$ of $w$ in $G_\ScS$}{
...
$\bS(A)$ and $\bS(B)$ in the SEBD graph $G_\ScS$, i.e., it returns
$\mu^* = \min_{\pi: A \rightsquigarrow B} \sum_{(u,v) \in \pi} \dS(u,v)$,
where the minimum is over all paths $\pi$ from $\bS(A)$ to $\bS(B)$ in $G_\ScS$.
\end{theorem}

\begin{proof}
We prove correctness in three parts: (a) the forward Dijkstra is correct for
shortest paths in $G_\ScS$; (b) the bidirectional combination does not introduce
...
$G_\ScS$. This is the standard Dijkstra invariant, which holds because all edge
...
\textbf{Graph structure.} The SEBD graph $G_\ScS$ has $n = |\mathcal{R}|$ vertices
...
$\HH_1$ and $\HH_2$ that are congruent up to isometry of $\ScS = [0,1]^3$.
\end{theorem}
...
of $\ScS$. The hologram $\HH_k$ consists of slices $(z_i^{(k)}, \text{slice}_i^{(k)})$
...
$\CM(A,B) = \dS(\bS(A), \bS(B))$. The S-entropy state space $\ScS = [0,1]^3$ is
a Riemannian manifold (a cube with the flat metric), and the map $A \mapsto \bS(A)$
embeds each region as a point in $\ScS$.
...
$T: \ScS \to \ScS$ (a rigid motion of $[0,1]^3$): if $\bS^{(2)}(\phi(A)) = T(\bS^{(1)}(A))$
...
are transformed by $T$ as embedded subsets of $\ScS$: $\text{slice}_i^{(2)} = T(\text{slice}_i^{(1)})$.
This is an isometry of $\ScS$.

Hence $\HH_1$ and $\HH_2$ are congruent up to the isometry $T$ of $\ScS$. \qed
\end{proof}
...
\[
  \abs{{S_k}_\eps(A) - \Sk(A)} \leq C_k \eps, \quad
  \abs{{S_t}_\eps(A) - \St(A)} \leq C_t \eps, \quad
  \abs{{S_e}_\eps(A) - \Se(A)} \leq C_e \eps,
\]
...
$\CM \approx 1$ (large distance in $\ScS$), while contacts between uniform blocks
...
\item \textbf{Learning the S-entropy metric}: The S-entropy state space uses a
  ...
  (e.g., a Riemannian metric on $\ScS$) from training data that assigns higher
