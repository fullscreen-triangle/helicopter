...
float localEntropy(sampler2D tex, vec2 uv, vec2 ts) {
  ...
  return H / 2.0; // normalise to [0,1]
}

void main() {
  vec2 ts = 1.0 / vec2(textureSize(u_image, 0));

  // S_k: local spectral entropy from 3x3 neighbourhood
  float S_k = localEntropy(u_image, v_uv, ts);

  // S_t: normalized frequency position (x-axis IS the frequency axis)
  float S_t = v_uv.x;

  // S_e: conservation constraint
  float S_e = clamp(1.0 - S_k - S_t, 0.0, 1.0);

  // Map to partition coordinates (n, l, m, s) like microscopy encoder
  float nF = clamp(S_k * u_nmax, 1.0, u_nmax);
  float n  = floor(nF);

  // Gradient for angular and magnetic components
