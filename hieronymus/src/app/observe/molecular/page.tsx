...
import { encodeMolecular, MOLECULAR_EXAMPLE } from '@/encoders/molecular';
...
export default function MolecularPage() {
  ...
  const processData = useCallback(
    (text: string) => {
      ...
      setEncodeError(null);
      try {
        const encoded = encodeMolecular(text);
        setEncodingInfo(
          `${encoded.metadata.originalSize} chars | CPU encode: ${encoded.metadata.encodingTime.toFixed(1)}ms`
        );
        ...
        observe(encoded.imageData, 'spectral');
      } catch (err) {
        ...
        <div className="border-r border-gray-800/50 overflow-y-auto lg:border-r-0 lg:border-b">
          <div className="p-4 border-b border-gray-800/50">
            ...
            <TextDropZone
              ...
              placeholder="Drop frequency data"
            />
            <div className="mt-3">
              <textarea
                value={textInput}
                ...
                placeholder="Paste frequency,amplitude data..."
                className="w-full h-24 bg-gray-900 border border-gray-800 rounded p-2 text-xs font-mono text-gray-400 resize-none focus:border-amber-400/50 focus:outline-none"
              />
            </div>
            <button
              onClick={handleLoadExample}
              className="mt-2 w-full text-xs text-amber-400 border border-amber-400/30 rounded px-3 py-1.5 hover:bg-amber-400/10 transition-colors"
            >
              Load Example (Raman Spectrum)
            </button>
            {encodeError && (
        ...
        <div className="flex items-center justify-center bg-[#050810] relative overflow-hidden">
          ...
          {imagePreview ? (
            ...
            <div className="text-center text-gray-700">
              ...
              <div className="text-sm">
                Drop a frequency data file or paste data in the left panel
              </div>
          ...
          {result && (
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2">
              <span className="text-[9px] text-amber-400 bg-dark/85 border border-gray-800 px-3 py-1 rounded-full backdrop-blur">
                Spectral Encoder
              </span>
