...
import { encodeSignal, SIGNAL_EXAMPLE } from '@/encoders/signal';
...
export default function SignalPage() {
  ...
  const processData = useCallback(
    (text: string) => {
      ...
      setEncodeError(null);
      try {
        const encoded = encodeSignal(text);
        setEncodingInfo(
          `${encoded.metadata.originalSize} samples | CPU encode: ${encoded.metadata.encodingTime.toFixed(1)}ms`
        );
        ...
        observe(encoded.imageData, 'signal');
      } catch (err) {
    ...
    <div className="min-h-[calc(100vh-140px)] flex flex-col">
      ...
      <div className="px-6 py-3 border-b border-gray-800/50 flex flex-col gap-3">
        <div className="flex items-center gap-4">
          <h1 className="text-sm font-semibold tracking-wider text-violet-400">
            SIGNAL ENCODER
          </h1>
        ...
        <div className="border-r border-gray-800/50 overflow-y-auto lg:border-r-0 lg:border-b">
          <div className="p-4 border-b border-gray-800/50">
            ...
            <TextDropZone
              ...
              placeholder="Drop time series data"
            />
            <div className="mt-3">
              <textarea
                value={textInput}
                ...
                placeholder="Paste comma-separated values or time,value pairs..."
                className="w-full h-24 bg-gray-900 border border-gray-800 rounded p-2 text-xs font-mono text-gray-400 resize-none focus:border-violet-400/50 focus:outline-none"
              />
            </div>
            <button
              onClick={handleLoadExample}
              className="mt-2 w-full text-xs text-violet-400 border border-violet-400/30 rounded px-3 py-1.5 hover:bg-violet-400/10 transition-colors"
            >
              Load Example (Multi-Frequency Signal)
            </button>
            {encodeError && (
        ...
        <div className="flex items-center justify-center bg-[#050810] relative overflow-hidden">
          {loading && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
            >
              <div className="text-violet-400 text-sm animate-pulse tracking-wider">
                Processing on GPU...
              </div>
            </motion.div>
          )}
          ...
          {imagePreview ? (
            ...
            <div className="text-center text-gray-700">
              ...
              <div className="text-sm">
                Drop a CSV file or paste time series values
              </div>
          ...
          {result && (
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2">
              <span className="text-[9px] text-violet-400 bg-dark/85 border border-gray-800 px-3 py-1 rounded-full backdrop-blur">
                Signal Encoder
              </span>
