...
class MultiModalLifeSciencesValidator:
    ...
    def __init__(self, public_dir: str = '../maxwell/public', output_dir: str = 'multi_modal_validation'):
    ...
    def _find_image_files(self) -> List[Path]:
        ...
        if not self.public_dir.exists():
            print(f"WARNING: Directory not found: {self.public_dir}")
            ...
            for alt in alternates:
                if alt.exists():
                    print(f"  ✓ Found images in: {alt}")
                    self.public_dir = alt
                    break
            else:
                print(f"  ✗ No image directory found. Please specify with --public-dir")
                return []
        ...
        print(f"Found {len(image_files)} life sciences images in {self.public_dir}")
        if len(image_files) == 0:
            print(f"  No images found. Supported formats: {', '.join(valid_extensions)}")
    ...
    def validate_all_images(self, max_images: int = None) -> Dict:
        ...
        if successful_results:
            ...
            print(f"  Check that:")
            print(f"    1. Image directory exists: {self.public_dir}")
            print(f"    2. Images are valid formats: .jpg, .png, .tif, etc.")
            print(f"    3. Images can be loaded by OpenCV")
...
def main():
    ...
    parser.add_argument(
        '--public-dir',
        type=str,
        default='../maxwell/public',
        help='Directory containing life sciences images (default: ../maxwell/public)'
    )
    parser.add_argument(
        '--output-dir',
        type=str,
