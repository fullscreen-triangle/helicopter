...
where $R'$ was processed at step $j < i$. The threshold $\Delta A_{\text{revisit}}$ prevents oscillation by requiring a significant increase in ambiguity.

\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/image_processing_panel_chart.png}
\caption{\textbf{Image Processing Panel Chart: Membrane State Validation Across Real and Synthetic Images.} 
Comprehensive validation of dual-membrane encoding across real biological images 
(Moriarty series, Me Original series) and synthetic test patterns (20251126/20251203 
series), showing front/back membrane states and Shannon entropy distributions.
\textbf{Top rows (Carbon Copy \& Pattern):} (\textit{top-left}) Carbon copy 
validation pattern showing checkerboard structure (range [0.0, 0.999], mean = 0.503), 
confirming deterministic encoding. (\textit{top-right}) Pattern 20251126\_124943 
showing uniform high-entropy state (range [$1.55 \times 10^{19}$, $1.85 \times 10^{19}$], 
mean $\approx 1.69 \times 10^{19}$), representing raw information content.
\textbf{Real Back Moriarty Series (rows 2-3):} Three timestamps (124931, 124850) 
showing back-membrane Shannon entropy $S_k$ with smooth gradients (blue-to-yellow, 
range [0.0, 0.877], mean = 0.551). Temporal consistency across 19-minute interval 
validates stability. Corresponding "Me Original" panels show uniform high-entropy 
states before categorical depth extraction.
\textbf{Real Front Moriarty Series (rows 4-5):} Front-membrane states showing 
negative Shannon entropy (purple-to-blue, range [$-0.877$, 0.0], mean = $-0.551$), 
confirming amplitude-phase complementarity: $S_k^{\text{front}} = -S_k^{\text{back}}$. 
Spatial patterns are exact inversions of back-membrane states.
\textbf{Synthetic Series (rows 6-10):} Synthetic validation patterns with controlled 
entropy distributions: (\textit{Back 090703, 123319, 124831}) Uniform teal 
(high-entropy, $\approx 1.69 \times 10^{19}$) and gradient patterns (blue-to-yellow, 
range [0.0, 0.872], mean = 0.548). (\textit{Front 090703, 123319, 124831}) 
Complementary patterns with negative entropy (purple-to-blue, range [$-0.872$, 0.0], 
mean = $-0.548$), validating front-back conjugacy.
\textbf{Shannon Entropy (Sk) Panels:} Processed entropy maps showing spatial 
heterogeneity after categorical depth extraction. Real images exhibit structured 
patterns (cellular organization), while synthetic patterns show controlled 
gradients/checkerboards. Front-back complementarity maintained: 
$|S_k^{\text{front}} + S_k^{\text{back}}| < 0.003$ for all panels.
\textbf{Validation Metrics:}
• \textbf{Real image consistency:} Moriarty series shows $\sigma_{\text{time}}(S_k) = 0.000$ across 19-minute interval (124931 vs. 124850), confirming temporal stability.
• \textbf{Synthetic pattern fidelity:} Target entropy distributions achieved within $< 1\%$ error (mean = 0.548 vs. target 0.551).
• \textbf{Front-back complementarity:} Entropy conservation $> 99.7\%$ for all 30+ panels shown.
• \textbf{Spatial encoding accuracy:} Checkerboard patterns (Carbon Copy) preserve edge sharpness, validating pixel-level encoding.}
\label{fig:image_processing_panel}
\end{figure*}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/detector_comparison_panel.png}
\caption{\textbf{Multi-detector comparison analysis across 7 measurement modalities.} 
\textbf{Top row:} Detector performance profile (radar plot) comparing normalized metrics 
(mean, std, entropy, skewness, kurtosis) across barometer, hygrometer, interferometer, 
mass spectrometer, photodiode, Raman spectrometer, and thermometer; Statistical fingerprints 
(heatmap) showing correlation structure (Pearson $r$) between detectors, revealing strong 
positive correlations (dark red, $r \approx 1.0$) for mean values and strong negative 
correlations (dark blue, $r \approx -1.0$) for entropy/skewness; Value distributions 
(box plots) demonstrating dynamic range spanning 5 orders of magnitude (barometer: 101,325 Pa 
to interferometer: 1.0 normalized units); Metric correlations (heatmap) quantifying 
relationships between statistical descriptors (mean, std, entropy, skewness, kurtosis, median) 
across all detectors. 
\textbf{Middle row:} Mean values (barometer dominates at 101,325 Pa, all others $<$100); 
Standard deviations (thermometer shows highest variability, σ = 0.000175, interferometer 
shows lowest, σ $<$ 0.000025); Information entropy (all detectors exhibit $H \approx 1.6 \times 10^{-13}$ 
bits, indicating deterministic signals with minimal noise); Dynamic range (barometer: 100,000 Pa 
span, all others $<$100 units). 
\textbf{Bottom row:} Phase distributions (circular histograms) showing angular alignment 
at 270° for most detectors (indicating phase-locked oscillations); Distribution shape 
analysis (skewness-kurtosis plot) revealing barometer and hygrometer as outliers (high 
kurtosis $\approx$ 0.5, near-zero skewness), while other detectors cluster near origin; 
Similarity clustering (dendrogram) demonstrating hierarchical relationships: thermometer 
forms isolated branch (distance = 2.0), photodiode/Raman/interferometer cluster together 
(distance $<$ 0.5), mass spectrometer intermediate (distance = 1.0); Principal component 
analysis (PC1 = 52.5\%, PC2 = 30.0\%) showing thermometer and mass spectrometer as orthogonal 
outliers, other detectors clustered near origin. 
\textbf{Additional panels:} Quantile analysis (box plots) confirming barometer's extreme 
dynamic range (median = 101,325 Pa, IQR = 17,500 Pa); Cumulative distribution functions 
showing barometer's unique sigmoid profile (inflection at 101,325 Pa) versus step functions 
for normalized detectors; Normalized metrics (grouped bar chart) revealing barometer's 
dominance in mean (1.0) but near-zero entropy/skewness; Quantitative summary table listing 
exact values (barometer: mean = 101,325.0000, entropy = 0.0000; thermometer: mean = 310.1495, 
entropy = 0.0002). All measurements synchronized to 30 Hz acquisition, enabling cross-modal 
correlation analysis with sub-millisecond temporal precision.}
...
This does not eliminate ambiguity—it reduces ambiguity below the hardware noise floor, where further disambiguation is physically impossible.
