...
% Figure reference shortcut removed
...
\emph{interaction} (photon propagation, limited by opacity and the inverse-square 
...
The Moon is observed nightly by terrestrial observers, appearing as a luminous disc subtending approximately 0.5 degrees of arc. Features on its surface—maria, craters, mountain ranges—are visible even to the unaided eye. Through telescopes, finer details emerge: rilles, individual craters, and, with sufficient resolution, human-made artefacts from the Apollo missions.

This observational reality poses fundamental questions: Why does the Moon exist as a stable, massive body at a distance of $\sim 384{,}400$ km from Earth? Why does it follow elliptical orbital dynamics? Why do images of the Moon exhibit particular resolution limits? Can subsurface structures be inferred from surface observations?

Standard physics addresses these questions through distinct frameworks: celestial mechanics for orbital dynamics, wave optics for image formation, and geophysics for subsurface structure. We demonstrate that all these phenomena emerge from a single principle: \textbf{categorical partitioning of bounded oscillatory systems}.
...
\begin{proof}
\textbf{Oscillatory derivation}: A bounded harmonic oscillator in $M$ dimensions with a characteristic frequency $\omega_0$ exhibits quantised energy levels. When partitioned to depth $n$, the system admits $n$ distinguishable oscillatory modes per dimension, yielding $n^M$ accessible microstates. Entropy:
\begin{equation}
...
Since all three yield identical expressions for arbitrary $M$ and $n$, they describe the same underlying structure. The equivalence is not analogical but a mathematical identity:
\begin{equation}
...
From the sequential partitioning of bounded systems, natural coordinates emerge.

\begin{definition}[Partition Coordinates]
\label{def:partition_coords}
A bounded oscillatory system admits parameterisation by partition coordinates $(n, \ell, m, s)$:
\begin{itemize}
...
\end{itemize}
These coordinates are not imposed but necessarily emerge from the sequential partitioning process.
\end{definition}
...
The oscillation frequency corresponds to partition coordinates through system geometry.

\begin{theorem}[Frequency-Depth Correspondence]
\label{thm:frequency_depth}
For a bounded oscillator with a fundamental frequency $\omega_0$ and a characteristic length $L$, partition depth $n$ corresponds to the oscillatory frequency:
\begin{equation}
\omega_n = n^2 \omega_0
\end{equation}
...
\begin{proof}
Sequential partitioning to depth $n$ creates a nested structure with a characteristic length scale:
\begin{equation}
\lambda_n = \frac{L}{n}
\end{equation}
...
However, partition capacity scales as $2n^2$, indicating that partition depth $n$ creates $n^2$ effective degrees of freedom. The energy associated with partition depth $n$ must account for all accessible states within that partition level.
...
Normalising to the fundamental frequency $\omega_0$ (corresponding to $n=1$, $E_1 = \hbar\omega_0$):
\begin{equation}
E_n = n^2 \hbar \omega_0
\end{equation}
...
The equivalence oscillation $\equiv$ category $\equiv$ partition enables a measurement modality fundamentally distinct from photon-based observations.
...
\begin{itemize}
    \item Partition coordinates parameterize categorical structure 
    \item Spatial coordinates parameterize geometric embedding (a separate structure)
    \item Optical properties govern photon propagation (a separate physical process)
\end{itemize}
...
\begin{itemize}
    ...
    \item Large $d_{\text{spatial}}$ but small $d_{\text{cat}}$ (spatially distant but categorically similar, e.g., identical atoms separated by metres)
...
The independence $d_{\text{cat}} \perp d_{\text{spatial}}$ is not a physical assumption requiring experimental validation but a mathematical consequence of how these metrics are defined on distinct spaces. Partition signatures exist independently of whether photons can propagate between systems.
\end{proof}
...
I(d) = I_0 e^{-\tau d}
\end{equation}
where $\tau$ is opacity and $d$ is penetration depth. For $\tau d \gg 1$, the signal becomes undetectable.

Categorical measurement accesses partition signatures $(n, \ell, m, s)$ directly through frequency synchronisation. Since $d_{\text{cat}} \perp \tau_{\text{optical}}$  opacity does not attenuate categorical access.

Subsurface structure at depth $d$ beneath opacity $\tau$ remains categorically accessible, provided its partition signature $(n, \ell, m, s)$ is distinguishable from the surrounding material. Detection is limited by partition distinguishability, not photon transmission.
\end{proof}

\begin{remark}
 The ability to detect structures beneath opaque media is not a violation of electromagnetic wave propagation but a consequence of measuring in categorical space rather than spatial-photonic space.
\end{remark}
...
The Schrödinger equation for a bounded system emerges as the effective description of partition completion dynamics. Quantum numbers $(n, \ell, m, s)$ are partition coordinates, not independent axioms.
\end{corollary}
...
\hat{H}\psi = E\psi
\end{equation}
admits solutions labelled by quantum numbers $(n, \ell, m, s)$ with energies $E_n$ and angular momentum quantum numbers $\ell, m$.

These are partition coordinates with $E_n = n^2 \hbar \omega_0$. The Schrödinger equation describes how partition states evolve, with the Hamiltonian $\hat{H}$ encoding partition completion rules.
...
Quantisation of energy, angular momentum, and spin emerges from partition distinguishability requirements. Continuous classical mechanics is the $n \to \infty$ limit where partition structure becomes unresolvable.
\end{corollary}

\begin{proof}
Partition coordinates $(n, \ell, m, s)$ are discrete by construction. Each partition state must be distinguishable from all others, requiring integer quantum numbers.

For large $n$, adjacent partition states have a small energy difference:
\begin{equation}
...
Physical measurement corresponds to accessing partition signatures $(n, \ell, m, s)$ of target systems. Resolution is limited by the partition depth ratio between the observer and target systems.
\end{corollary}

\begin{proof}
An observer system with partition depth $n_{\text{obs}}$ can distinguish partition states with $d_{\text{cat}} \gtrsim 1/n_{\text{obs}}$. Target systems with a partition depth $n_{\text{target}} \gg n_{\text{obs}}$ have fine structures that are unresolvable to the observer.
...
These corollaries establish that standard quantum mechanics is not fundamental but emerges from a categorical partition structure. This framework extends beyond quantum mechanics to include classical, relativistic, and thermodynamic phenomena within a unified description. The remainder of this work demonstrates that astronomical observation—including the existence of the Moon, orbital mechanics, image formation, and subsurface detection—follows from these principles.
...
The categorical description reveals a structure invisible to purely oscillatory observation. While oscillatory measurements access kinetic properties (energies, frequencies, amplitudes), categorical measurements access topological properties (network connectivity, morphism structure, and partition relationships).
...
\begin{itemize}
    ...
    \item \textbf{Morphisms}: Structural transformations $\Phi: \Sigma_A \to \Sigma_B$ that preserve partition relationships (conservation laws, symmetries)
...
\end{enumerate}
These faces are complementary, with the uncertainty relation:
\begin{equation}
...
\begin{proof}
The kinetic face corresponds to time evolution $\partial/\partial t$ applied to dynamical variables. Measurements access instantaneous values of position, momentum, and energy at specific times $t$.
...
The simultaneous precise measurement of kinetic energy $E = \hbar\omega$ and categorical completion time $\tau$ is constrained by:
\begin{equation}
...
Phase-lock networks encode categorical structures in physical configurations.

\begin{definition}[Phase-Lock Network]
\label{def:phase_lock_network}
A \textbf{phase-lock network} is a coupling structure where oscillators at positions $\mathbf{r}_i$ with phases $\phi_i$ synchronise through interaction potentials:
\begin{equation}
...
\begin{proof}
Phase-lock coupling depends on:
\begin{itemize}
...
Categorical information resides in network topology, not in kinetic observables. A molecule at 300 K and the same molecule at 400 K has identical categorical structures despite having different kinetic energies.
\end{proof}
...
\begin{remark}
This definition is equivalent to the partition coordinate metric but emphasizes the morphism structure. For partition signatures differing by $\Delta n$ in principal quantum number, $\Delta\ell$ in angular momentum, etc., the morphism chain length scales as:
\begin{equation}
...
These coordinate systems are mathematically independent. To demonstrate statistical independence, consider concrete examples:
...
Dissimilar atoms separated macroscopically (iron on Earth, helium on the Moon):
\begin{itemize}
    \item Physical distance: $d_{\text{spatial}} \sim 3.8 \times 10^8$ m
...
All four combinations are physically realisable, demonstrating that $d_{\text{spatial}}$ and $d_{\text{cat}}$ vary independently. The correlation coefficient:
\begin{equation}
...
If the subsurface structure has a distinct partition signature (e.g., a rock layer with a different composition than the overlying regolith), it remains categorically accessible even when photonically inaccessible.

This enables "see-through" observation: physically distant or obscured structures can be categorically accessible if $d_{\text{cat}}$ is small (short morphism chains available) or if intermediate partition stages provide catalytic pathways.
\end{proof}

\begin{remark}
This is the key theoretical result enabling subsurface lunar imaging. Bootprints at 3.5 cm depth and rock layers at 2.3 m depth are photonically inaccessible (regolith is opaque) but categorically accessible (distinct partition signatures propagate through phase-lock network continuity).
\end{remark}
...
An \textbf{information catalyst} is a categorical structure $C$ that creates intermediate partition stages:
\begin{equation}
...
More generally, catalysts enable access to partition states that are not directly connected by allowed morphisms. If no direct morphism $\Phi: \Sigma_A \to \Sigma_B$ exists (e.g., due to conservation law constraints), but a catalysed path exists through intermediate states respecting all conservation laws, then:
\begin{equation}
...
Multi-aperture interferometry acts as an information catalyst:
\begin{itemize}
...
reduces categorical distance $d_{\text{cat}}(\Sigma_{\text{telescope}}, \Sigma_{\text{target}})$, enabling the resolution of fine lunar surface features beyond single-aperture diffraction limits.

This mechanism extends to virtual interferometry: computational apertures serve as information catalysts without requiring physical telescope construction.
\end{example>
...
The key results are:
\begin{itemize}
    \item Categorical distance $d_{\text{cat}}$ is independent of spatial distance $d_{\text{spatial}}$ and optical opacity $\tau_{\text{optical}}$
    \item Phase-lock networks encode categorical structure independently of kinetic energy
    \item Information catalysis reduces categorical distance through intermediate partition stages
\end{itemize}
...
\begin{proof}
Angular complexity $\ell$ counts angular nodes in oscillatory amplitude distribution. For a bounded system, nodes must be spatially arranged. The number and arrangement of nodes defines dimensionality.
...
\begin{proof}
Partition depth $n$ counts the number of distinguishable states in a system. For a system with $N$ particles, each with partition depth $n_{\text{particle}}$:
\begin{equation}
...
Observing structures at partition depth $n_{\text{target}}$ requires observer partition depth $n_{\text{obs}} \gtrsim n_{\text{target}}$ or catalytic enhancement.
\end{corollary}

\begin{proof}
Measurement resolution is bounded by:
\begin{equation}
\Delta n \sim \frac{n_{\text{target}}}{n_{\text{obs}}}
\end{equation}
...
Still insufficient. Virtual interferometry uses computational apertures to achieve $n_{\text{eff}} \sim 10^{30}$, enabling lunar surface resolution at cm scale.
\end{proof}
...
\end{equation}
This is the time-energy uncertainty relation applied to partition completion dynamics.
\end{theorem}
...
For virtual interferometry using CPU oscillators, the relevant timescale is the CPU clock period:
\begin{equation}
...
\begin{enumerate}
    \item \textbf{Space emerges from partition geometry}: Three-dimensional Euclidean space is not assumed but derived from angular partition coordinates $(\ell, m)$ and radial depth $n$.
    
    \item \textbf{Physical surfaces are partition boundaries}: The Moon's surface is a discontinuity in partition structure, with interior partition depth $n_{\text{interior}} \sim 10^{51}$ dropping to exterior $n_{\text{exterior}} \sim 1$.
    
    \item \textbf{Observation requires partition depth matching}: Resolving lunar surface features at cm scale requires observer partition depth $n_{\text{obs}} \sim 10^{30}$, achievable through interferometric or catalytic enhancement.
    
    \item \textbf{Temporal resolution is detector-limited}: Fundamental partition lag $\tau_{\text{lag}} \sim 10^{-15}$ s is far shorter than practical detector lag $\sim 10^{-6}$ s, so temporal resolution is not a fundamental constraint.
\end{enumerate}

These results establish the geometric framework for astronomical observation. The Moon exists as a massive body with partition depth $n \sim 10^{51}$, bounded by a sharp surface (partition boundary), observable from Earth through partition signature access requiring effective partition depth $n_{\text{eff}} \sim 10^{30}$ achievable via virtual interferometry.
...
\begin{proof}
Observation accesses partition signatures $(n, \ell, m, s)$. Accessing a signature requires the partition boundary to be determinate—the system must be in a definite categorical state, not an undetermined residue.
...
This is irreversible, establishing that observation creates temporal ordering. The "collapse" of quantum superposition is the crystallisation of partition boundaries through measurement-induced completion.
\end{proof}
...
\end{equation}
Spatial coordinates $(x,y,z)$ arise from partition structure $(n,\ell,m)$; the temporal coordinate $t$ arises from the categorical completion sequence.
\end{theorem}

\begin{proof}
Spatial coordinates emerge from the angular partition structure:
\begin{align}
r &\sim n a_0 \quad \text{(radial from partition depth)} \\
(\theta, \phi) &\sim (\ell, m) \quad \text{(angular from node arrangement)}
\end{align}

Temporal coordinates emerge from the completion order:
\begin{equation}
...
\end{equation}
where $c$ is the maximum partition completion propagation speed—the speed at which partition boundaries can crystallise across space.
...
\begin{proof}
Partition completion at one location influences partition completion at nearby locations through phase-lock network coupling. The coupling propagates at finite speed determined by the oscillatory frequency and wavelength:
\begin{equation}
...
\begin{proof}
Phase-lock coupling strength has form:
\begin{equation}
...
\begin{proof}
Partition depth scales with mass:
\begin{equation}
...
(These are total partition depths accounting for all particles. Surface partition depths relevant for observation are lower, $n_{\text{surface}} \sim 10^{30}$.)
...
\end{equation}
where $T$ is kinetic energy and $V_{\text{grav}}$ is gravitational potential.
\end{corollary}
...
\begin{enumerate}
    \item \textbf{Time emerges from partition completion order}: Temporal direction is determined by entropy increase through partition boundary crystallization. Time is not pre-existing but relational.
    
    \item \textbf{Space-time is unified partition structure}: Four-dimensional space-time emerges from partition coordinates $(n, \ell, m)$ and completion ordering. Special relativity follows from partition propagation limits.
    
    \item \textbf{Gravity emerges from phase-lock networks}: Gravitational interaction is the long-range tail of phase-lock coupling, with strength determined by partition capacity (mass). Newton's law of gravitation is recovered.
    
    \item \textbf{Astronomical systems organize hierarchically}: Sun $\to$ Earth-Moon $\to$ individual bodies form nested partition structures. Orbital dynamics emerges from phase-lock equilibrium.
\end{enumerate}
...
\begin{proof}
Partition depth characterizes the number of distinguishable states in a system. For a single atom, $n_{\text{atomic}} \sim 1$--$100$ depending on atomic number (number of electrons, filled shells).
...
For resolution $\lambda_{\text{resolution}} \sim 1$ cm achievable through virtual interferometry:
\begin{equation}
...
\begin{proof}
Phase-lock network establishes gravitational coupling $V(r) = -GM_1 M_2/r$. For stable circular orbit, the partition gradient (gravitational "force") balances the centripetal acceleration:
\begin{equation}
...
\begin{proof}
The gravitational potential at a distance $r$ from the Moon's centre:
\begin{equation}
...
\begin{proof}
The Moon is not a point mass but an extended body with a mass distribution. The gravitational coupling varies across the Moon's diameter:
\begin{itemize}
...
At synchronous rotation, $\tau_{\text{tidal}} = 0$ and the system are in a minimum energy configuration. The tidal bulge remains aligned with the Earth-Moon axis, eliminating energy dissipation.

\textbf{Observational confirmation:} The Moon always presents the same face to Earth (except for small libration effects due to orbital eccentricity and axial tilt). The rotation period equals the orbital period with high precision:
\begin{equation}
...
\begin{itemize}
    \item All major moons of Jupiter, Saturn, Uranus, and Neptune are tidally locked
    \item Mercury is in a 3:2 spin-orbit resonance with the Sun
    \item Pluto and Charon are mutually tidally locked
\end{itemize}
This is not a coincidence but the inevitable outcome of phase-lock network energy minimisation over long timescales.
\end{remark}
...
\end{equation}
where $A_{\ell m}$ are partition amplitudes encoding topographic features (craters, mountains, maria) and $Y_\ell^m$ are spherical harmonics.
\end{theorem}

\begin{proof}
The partition boundary is not perfectly spherical but has local variations due to:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Large craters}: Localised depressions with a depth of $\sim 1$--$10$ km and a diameter of $\sim 10$--$1000$ km
    \item \textbf{Small craters}: Depth of $\sim 1$--$100$ m, diameter of $\sim 10$--$1000$ m
...
These signatures are distinguishable through categorical measurement, enabling identification even when spatial resolution is insufficient to resolve individual features photonically.
\end{proof}
...
\begin{enumerate}
    \item \textbf{Mass and size}: $M = 7.34 \times 10^{22}$ kg, $R = 1737$ km emerge from a stable partition configuration with $n_{\text{eff}} \sim 10^{17}$
    
    \item \textbf{Orbital radius}: $r = 384{,}400$ km is the equilibrium distance of the Earth-Moon phase-lock network, derived from $T = 27.3$ days
    
    \item \textbf{Orbital period}: $T = 27.3$ days is the categorical completion time for one Earth-Moon cycle
    
    \item \textbf{Surface gravity}: $g = 1.62$ m/s$^2$ emerges from the partition gradient at the surface
    
    \item \textbf{Tidal locking}: Synchronous rotation ($T_{\text{rotation}} = T_{\text{orbital}}$) minimises the phase-lock network energy
    
    \item \textbf{Topography}: Surface features are partition boundary variations with distinct categorical signatures
\end{enumerate}
...
Measurement resolution is bounded by partition depth ratio:
\begin{equation}
...
\begin{remark}
This establishes that image formation is a categorical measurement process, not a geometric projection. The image does not "show" the Moon directly but represents the categorical overlap between lunar partition structure and detector partition structure.
\end{remark}
...
Telescopic resolution is fundamentally limited by the partition depth ratio between the detector and the target.

\begin{theorem}[Resolution Limit from Detector Partition Depth]
\label{thm:resolution_limit_derived}
For a detector with an aperture diameter $D$ and wavelength $\lambda$ observing a target at a distance $r$, the minimum resolvable feature size is:
\begin{equation}
...
Angular resolution from partition geometry:
\begin{equation}
...
A 21 km aperture telescope is physically infeasible. This is why conventional imaging cannot resolve cm-scale lunar features from Earth. However, categorical measurement bypasses this limitation by accessing partition signatures directly, not through photonic spatial resolution.
\end{remark}
...
\end{equation}
where each $\mathcal{I}_{t_k}$ is a categorical projection at completion time $t_k$.
\end{definition}
...
\begin{itemize}
    ...
    \item \textbf{Grain size}: Regolith grain size affects scattering efficiency
        \begin{itemize}
            \item Fine grains ($< 100$ $\mu$m): Multiple scattering, higher albedo
...
\begin{enumerate}
    \item \textbf{Images are categorical projections}: Not geometric copies, but projections of target partition signatures onto detector partition spaces
    
    \item \textbf{Angular size determines pixel count}: Moon subtends $\theta \approx 0.52^\circ$, determining how many pixels resolve lunar features
    
    \item \textbf{Resolution is partition depth limited}: The diffraction limit $\delta x = \lambda r/D$ emerges from detector partition depth $n \sim D/\lambda$
    
    \item \textbf{HST cannot resolve Apollo artifacts}: Resolution $\sim 106$ m, insufficient for landers ($\sim 9$ m) or flags ($\sim 0.9$ m)
    
    \item \textbf{Videos are temporal sequences}: A phase cycle of $T = 29.53$ days represents Earth-Moon-Sun partition completion
    
    \item \textbf{Albedo encodes composition}: Maria ($A \sim 0.07$) vs. highlands ($A \sim 0.12$) are distinct partition signatures
\end{enumerate}

These results establish that conventional lunar images and videos are categorical representations of partition structure, limited by detector partition depth. To achieve cm-scale resolution from Earth requires bypassing photonic diffraction limits through categorical measurement—the subject of the next sections.
...
From partition depth, angular resolution follows:
\begin{equation}
...
\begin{proof}
Each aperture provides partition coordinates up to depth $n_k$. Each aperture can distinguish $2n_k^2$ states.
...
Information catalysts create intermediate partition stages:
\begin{equation}
...
\textbf{Catalyst 1 - Surface texture priors:} Known regolith grain statistics constrain sub-resolution structure. Lunar regolith has characteristic grain size distribution ($\sim 10$--$100$ $\mu$m), packing density ($\sim 50\%$), and roughness statistics. These priors propagate partition information from coarse to fine scales:
\begin{equation}
...
Each catalyst reduces categorical distance $d_{\text{cat}}$, making finer partition structures accessible through morphism chains even without additional photon collection.
...
\begin{itemize}
    \item \textbf{Information content}: Cannot extract more information than is present in the data
...
Starting from interferometric resolution $\delta x_{\text{interfero}} = 2.1$ cm, applying three information catalysts with $\gamma = 3$ each:
\begin{equation}
...
\begin{proof}
Oscillatory frequency $\omega = 2\pi c/\lambda$ determines partition angular complexity:
\begin{equation}
...
Interferometric fringe visibility requires partition lag $\tau_{\text{lag}}$ to be less than the observation time per fringe:
\begin{equation}
...
\begin{enumerate}
    \item \textbf{Telescopes have partition depth} $n \sim D/\lambda$: Resolution emerges from partition geometry, not wave diffraction
    
    \item \textbf{Interferometry enhances partition depth} to $n_{\text{eff}} \sim B/\lambda$: Baseline $B$ replaces aperture $D$
    
    \item \textbf{10 km baseline achieves 2.1 cm resolution}: Sufficient to resolve Apollo landers and flags
    
    \item \textbf{Virtual resolution reaches 0.78 mm}: Information catalysis provides $27\times$ enhancement, resolving bootprint treads
    
    \item \textbf{Spectroscopy accesses different partition coordinates}: Multispectral imaging provides $\sqrt{N_{\lambda}}$ enhancement
    
    \item \textbf{Partition lag is not limiting}: Coherence maintained at optical frequencies; environmental factors dominate
\end{enumerate}
...
Combined, these techniques enable resolution exceeding orbital imaging (LRO: $\sim 0.5$ m) from a distance of Earth (384,400 km), achieved through categorical measurement rather than photonic imaging.
...
For $d_{\text{char}} = 1$ mm and typical grain size $d_{\text{grain}} = 50$--$100$ $\mu$m:
\begin{equation}
...
\begin{itemize}
    \item \textbf{Grain size}: $d_{\text{grain}} \sim 50$--$100$ $\mu$m typical (Apollo 11, 12, 14--17 samples)
...
\begin{itemize}
    \item \textbf{Apollo 11} (Mare Tranquillitatis): Core depth 2.3 m, grain size 45--90 $\mu$m, density 1.5--1.7 g/cm$^3$
    \item \textbf{Apollo 12} (Oceanus Procellarum): Core depth 2.4 m, grain size 50--100 $\mu$m, density 1.6--1.8 g/cm$^3$
    \item \textbf{Apollo 15} (Hadley Rille): Core depth 2.9 m, grain size 40--80 $\mu$m, density 1.4--1.7 g/cm$^3$
    \item \textbf{Apollo 16} (Descartes Highlands): Core depth 3.0 m, grain size 60--120 $\mu$m, density 1.7--1.9 g/cm$^3$
    \item \textbf{Apollo 17} (Taurus-Littrow): Core depth 3.1 m, grain size 50--100 $\mu$m, density 1.5--1.8 g/cm$^3$
\end{itemize}
...
For structure at depth $z$ beneath the surface, partition signature can be inferred via the catalytic morphism chain:
\begin{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Phase-lock network continuity}: Grain-grain contacts create a continuous network from surface to depth
    \item \textbf{Thermodynamic constraints}: temperature and pressure gradients determined by surface conditions
    \item \textbf{Seismic coupling}: Elastic wave propagation (if vibration data is available)
    \item \textbf{Electromagnetic coupling}: Radar penetration (for wavelengths $\lambda \gtrsim 1$ cm)
\end{enumerate}
\end{theorem}

\begin{proof}
Visible light penetrates regolith only $\sim 10^{-6}$ m (absorption length $\alpha^{-1} \sim 1$ $\mu$m). Direct photonic imaging of subsurface is impossible. However, partition signatures propagate through catalytic morphisms.
...
For depths $z \lesssim 5$ m (typical regolith thickness at Apollo sites), $d_{\text{cat}} \lesssim 50$. Morphism chains with $K \lesssim 50$ are categorically accessible, making subsurface partition structure inferrable despite physical opacity to visible photons.
\end{proof}
...
Applying see-through imaging to the Apollo flag locations reveals subsurface structure.
...
Grain size correlates with composition: TiO$_2$-rich basalt has finer grains (50--70 $\mu$m) than anorthosite (70--100 $\mu$m).
...
If surface resolution is $\delta x_0 = 1$ cm from virtual interferometry, subsurface resolution at depth $z = 3$ m is:
\begin{equation}
...
\begin{enumerate}
    \item \textbf{Apollo artifacts have distinct partition signatures}: Flags ($n \sim 12$, $\ell = 2$), LM ($n \sim 15$, $\ell = 4$), footprints ($\Delta n \sim 2$) are categorically distinguishable from natural features
    
    \item \textbf{Regolith has characteristic partition structure}: Grain size 50--100 $\mu$m, thickness 2--8 m, density 1.5--1.9 g/cm$^3$
    
    \item \textbf{Subsurface structure is categorically accessible}: See-through imaging via catalyst chains (conservation, phase-lock, thermodynamic, seismic, EM)
    
    \item \textbf{Beneath-flag structure is inferrable}: Disturbed regolith (0--10 cm), undisturbed regolith (10 cm--3 m), bedrock (> 3 m), validated by Apollo cores
    
    \item \textbf{Resolution degrades with depth}: $\delta x(z) = \delta x_0 \exp(\alpha d_{\text{cat}})$, giving $\sim 4.5$ cm at 3 m depth
    
    \item \textbf{Confidence is quantifiable}: 81\% with 5 independent catalysts, $> 90\%$ with Apollo ground truth
\end{enumerate}
...
Having established see-through imaging capability, we now demonstrate the ability to quantify physical properties such as volumes, masses, and energies from partition signatures alone, without direct physical measurement.

\subsection{Motivation: From Qualitative to Quantitative}

The subsurface partition inference provides structural information about the lunar subsurface. We now extend this to quantitative physical properties by:
...
\begin{proof}
The Apollo A7L spacesuit boots have dimensions $L = 0.30$ m and $W = 0.10$ m. From the partition depth analysis, the average depression is $\delta = 0.03$ m.
...
Using the regolith density from the compositional analysis, the total mass displaced is:
\begin{equation}
...
Having derived the Moon's position, mass, and orbital dynamics from partition geometry, we now demonstrate \textbf{predictive celestial mechanics} by calculating where on Earth's surface solar eclipse shadows fall, while validating against over 50 years of NASA historical eclipse data.
...
This rapid transition causes detectable changes in partition signatures from Earth: the thermal infrared emission drops, and the reflected Earth-light becomes the dominant component. $\square$
\end{proof}
...
\begin{itemize}
    \item Exoplanet transit timing (analogous to lunar eclipses) is predictable
...
We have derived the Moon's physical properties and observational characteristics from the oscillation $\equiv$ category $\equiv$ partition equivalence without any additional assumptions.The mass $M_{\text{Moon}} = 7.342 \times 10^{22}$ kg arises from a stable partition configuration with an effective depth $n_{\text{eff}} \sim 10^{17}$.


\textbf{The Moon, as a physical reality, emerges from its partition structure.} Mass $M_{\text{Moon}} = 7.342 \times 10^{22}$ kg arises from a stable partition configuration with an effective depth $n_{\text{eff}} \sim 10^{17}$. The orbital radius $r \approx 384{,}400$ km follows from the Earth-Moon phase-lock network equilibrium. Orbital period $T = 27.3$ days represents the categorical completion cycle of the Earth-Moon-Sun configuration. Surface gravitational acceleration $g_{\text{Moon}} = 1.62$ m/s$^2$ emerges from the partition gradient at the lunar surface.

\textbf{Observation of the Moon follows from categorical projection.} Angular size $\theta \approx 0.52°$ results from the geometric projection of the lunar partition boundary onto the celestial sphere. Resolution limit $\delta x_{\min} = \lambda r/D$ emerges from detector partition depth $n_{\text{det}} \sim D/\lambda$. Interferometric enhancement achieves effective partition depth $n_{\text{eff}} = \sqrt{\sum_k n_k^2}$ through a multi-modal combination. Subsurface imaging proceeds via categorical distance reduction through information catalysis.
...
The Apollo missions (1969–1972) provide ground truth for validation. Predicted partition signatures match observed features quantitatively.

Regolith grain size predicted from partition depth $n_{\text{regolith}} \sim 10$--$20$ corresponds to the observed grain diameter $d \sim 50$--$100$ $\mu$m from Apollo sample analysis. The flagpole partition signature $(n = 12, \ell = 2, m = 0)$ matches the observed vertical cylindrical structure with a height of $\sim 1.5$ m. The Lunar Module descent stage signature $(n = 15, \ell = 4, m = \pm 2)$ corresponds to the observed octagonal structure with four-fold rotational symmetry. Bootprint partition depth change $\Delta n \sim 2$ predicts compression depth $\sim 3$--$4$ cm, matching photographic measurements. Subsurface rock detection at partition depth $n = 18$ corresponds to TiO$_2$-rich basalt bedrock at a measured depth of $2.3$ m (Apollo 11 core samples).

Agreement between predicted partition signatures and Apollo observations confirms that the categorical framework correctly describes the lunar surface structure.

\subsection{Resolution Enhancement Beyond Diffraction Limits}

Standard optical telescopes achieve a resolution of $\delta x \sim 100$ m for lunar observation from Earth. The Hubble Space Telescope, with an aperture of $D = 2.4$ m at a visible wavelength of $\lambda = 550$ nm, provides a resolution of $\delta x \approx 106$ m, which is insufficient to resolve Apollo artefacts.

Single-aperture resolution $\delta x_{\min} = \lambda r/D$ with $D = 2.4$ m yields $\delta x \sim 100$ m. Interferometric enhancement with a baseline of $B = 10$ km achieves an effective partition depth of $n_{\text{eff}} \sim B/\lambda \sim 1.8 \times 10^{10}$, giving a resolution of $\delta x_{\text{interfero}} \sim 2.1$ cm. Virtual super-resolution through information catalysis with an enhancement factor of $\gamma^K = 27$ (three catalysts with $\gamma = 3$ each) achieves $\delta x_{\text{virtual}} \sim 0.78$ mm.
...
The most significant result is the inference of subsurface structure via partition signature propagation with zero photon transmission through the intervening regolith.

The mechanism exploits conservation laws (mass, charge, energy continuity) and phase-lock network continuity to propagate partition signatures from surface to depth. Information catalysts reduce categorical distance through morphism chains: $\Sigma_{\text{surface}} \xrightarrow{C_1} \Sigma_1 \xrightarrow{C_2} \Sigma_2 \xrightarrow{C_3} \cdots \xrightarrow{C_K} \Sigma_{\text{subsurface}}$. Each catalyst exploits physical constraints (composition continuity, thermal gradients, seismic velocity profiles, electromagnetic penetration) to bridge categorical distance without photon transmission.

Apollo core samples validate the inferred subsurface structure. Regolith depth predicted as $2$--$3$ m from surface partition signatures matches the measured depth $2.3$ m at the Apollo 11 site. Rock composition predicted as TiO$_2$-rich basalt from surface spectral morphisms matches the chemical analysis of returned samples. The compaction gradient predicted as $\Delta\rho/\rho \sim 15\%$ density increase with depth matches the measured range $12$--$18\%$ from core tube resistance.
...
Physical observation requires photons to traverse from the target to the detector. Opacity blocks physical observation. Visible light penetrates lunar regolith only $\sim 1$ $\mu$m (absorption length $\alpha^{-1} \sim 10^{-6}$ m), making the subsurface physically opaque. Categorical observation requires partition signatures accessible via morphism chains. Categorical distance $d_{\text{cat}}$, not physical opacity, determines observability. For lunar subsurface at a depth of $z \sim 3$ m, categorical distance of $d_{\text{cat}} \sim 15$ stages is achieved via conservation and phase-lock catalysts. Therefore, the subsurface is physically opaque but categorically transparent.
...
Exoplanet surface imaging requires a resolution of $\delta x \sim 10^{-10}$ arcsec for Earth-sized planets at a distance of $10$ pc, which is impossible for single telescopes. Interferometric arrays with a baseline of $B \sim 1000$ km achieve this resolution, but virtual super-resolution via partition signature morphisms enables surface feature characterisation without kilometre-scale baselines. Stellar interior composition inferred from surface spectroscopy through categorical morphisms propagates surface abundance measurements to core conditions via nucleosynthesis constraints. Galactic dark matter inferred from rotation curves represents partition configurations that are photon-decoupled but categorically accessible via gravitational phase-lock networks.
...
Comparison to Apollo mission data demonstrates quantitative agreement across multiple observables. Lunar surface partition signatures match measured grain sizes ($50$--$100$ $\mu$m), composition (TiO$_2$-rich basalt), and compaction profiles ($12$--$18\%$ density increase with depth). Artefact partition signatures match observed locations, orientations, and geometric properties of flags, Lunar Modules, and equipment. Subsurface rock depths and compositions match inferred values from surface morphologies within $10$--$20\%$ uncertainty. Bootprint depths match predicted partition depth changes ($\Delta n \sim 2$ corresponding to $3$--$4$ cm compression). This agreement confirms that categorical partitioning correctly describes lunar structure at scales from millimetres to kilometres.

\subsection{Broader Implications}

This work establishes that astronomical observation—imaging distant objects, resolving fine structure, inferring obscured features—follows from categorical partitioning. There is no separate theory of optics, no separate theory of orbital mechanics, no separate theory of image formation. All emerge from oscillation $\equiv$ category $\equiv$ partition equivalence.

Physical reality (the Moon) and its observation (lunar images) are dual aspects of partition structure. The Moon is a partition configuration in spatiotemporal coordinates. Images are projections of that configuration onto detector partition spaces. The distinction between observable and unobservable reduces to categorical distance $d_{\text{cat}}$. Structures with small categorical distance are observable through short morphism chains. Structures with large categorical distances require many intermediate stages but are not fundamentally unobservable.
